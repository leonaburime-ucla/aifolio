...
from shared.google_gemini import DEFAULT_MODEL_ID, get_model


def _stringify_content_parts(raw: list[Any]) -> str:
    """Flatten structured LLM content parts into one parseable string."""
    parts = []
    for item in raw:
...
def _json_object_slice(text: str) -> str:
    """Extract the most likely top-level JSON object span from text."""
    start = text.find("{")
    end = text.rfind("}")
...
def _build_prompt(
    ...
    non_chart_response: Dict[str, Any] | None,
    dataset_metadata: Dict[str, Any] | None,
    conversation_history: List[Dict[str, Any]] | None,
) -> str:
    """Assemble the analyst prompt with optional follow-up context."""
...
def interpret_analysis(
    ...
    prompt = _build_prompt(
        ...
        data_scientist_message=data_scientist_message,
        charts=charts,
        non_chart_response=non_chart_response,
