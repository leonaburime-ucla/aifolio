...
from shared.google_gemini import DEFAULT_MODEL_ID
...
def _build_tool_call_prompt(
    ...
    """Build the exact tool-call prompt sent to the LLM router."""
    return (
        f"{message}\n\n"
...
def _select_tool_call(tool_calls: List[Dict[str, Any]], tool_name: str) -> Optional[Dict[str, Any]]:
    """Pick the router call or direct tool call emitted by the model."""
...
def _stringify_content_parts(raw: List[Any]) -> str:
    """Flatten structured content blocks into a single parseable string."""
    parts = []
    for item in raw:
...
def _strip_json_fence(text: str) -> str:
    """Remove a surrounding markdown code fence from JSON-like text."""
    if not text.startswith("```"):
        return text
    stripped = text.strip("`")
    return stripped.replace("json", "", 1).strip()


def _json_object_slice(text: str) -> str:
    """Extract the most likely top-level JSON object from surrounding text."""
    start = text.find("{")
    end = text.rfind("}")
...
def _looks_like_id_column(column_name: str) -> bool:
    """Detect identifier columns that should usually be excluded from features."""
...
def _infer_chart_kind_for_tool(tool_name: str) -> str:
    """Infer default chart kind for a tool name."""
    if tool_name in {"pca_transform", "truncated_svd", "fast_ica", "nmf_decomposition"}:
        return "pca"
    if tool_name == "pls_regression":
        return "plsr"
    if tool_name == "tsne_embedding":
        return "embedding"
    if "clustering" in tool_name:
        return "clustering"
    if "classification" in tool_name:
        return "classification"
    return "regression"


def _build_fallback_tool_calls(message: str) -> list[dict[str, Any]]:
    """Build deterministic fallback tool calls from user text."""
    lower = message.lower()
    fallback_tools: list[str] = []
    for key, tool in FALLBACK_MESSAGE_TOOL_MAP.items():
        if key in lower:
            fallback_tools.append(tool)
    unique_tools = list(dict.fromkeys(fallback_tools))
    return [
        {
            "tool_name": tool_name,
            "tool_args": {},
            "chart_kind": _infer_chart_kind_for_tool(tool_name),
        }
        for tool_name in unique_tools
    ]


def _resolve_tool_name(tool_name: Any, chart_kind: Any) -> Optional[str]:
    """Resolve canonical tool name from planner output aliases."""
    normalized_name = TOOL_NAME_ALIASES.get(tool_name, tool_name)
    if not normalized_name and chart_kind in {"pca", "plsr"}:
        return "pca_transform" if chart_kind == "pca" else "pls_regression"
...
def run_data_scientist_tool(
    ...
    prompt = _build_tool_call_prompt(message, tool_name, tool_args, tools_catalog)
    ...
    selected = _select_tool_call(tool_calls, tool_name)
...
def _load_manifest() -> List[Dict[str, Any]]:
    ...
    """
    return ds_datasets.load_manifest(SAMPLE_DATA_DIR)
...
def _resolve_dataset_entry(dataset_id: str) -> Optional[Dict[str, Any]]:
    ...
    """
    return ds_datasets.resolve_dataset_entry(dataset_id, _load_manifest())
...
def _detect_delimiter(handle) -> str:
    ...
    """
    return ds_datasets.detect_delimiter(handle)
...
def _read_csv_rows(path: Path) -> List[Dict[str, Any]]:
    ...
    """
    return ds_datasets.read_csv_rows(path)
...
def _read_xlsx_rows(path: Path) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of row dictionaries.
    """
    return ds_datasets.read_xlsx_rows(path)
...
def _read_xls_rows(path: Path) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of row dictionaries.
    """
    return ds_datasets.read_xls_rows(path)
...
def _load_dataset_rows(file_path: Path) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of row dictionaries.
    """
    return ds_datasets.load_dataset_rows(file_path)
...
def _parse_json_response(raw: Any) -> Dict[str, Any]:
    ...
    if isinstance(raw, list):
        raw = _stringify_content_parts(raw)
    cleaned = _strip_json_fence(str(raw).strip())
    try:
        return json.loads(cleaned)
    except json.JSONDecodeError:
        candidate = _json_object_slice(cleaned)
        if not candidate:
            return {}
        try:
...
def _format_ds_conversation_history(history: List[Dict[str, Any]]) -> str:
    ...
    return ds_planner.format_conversation_history(history)


def _invoke_planner_prompt(prompt: str, model_id: str) -> Any:
    llm = get_data_scientist_agent(model_id=model_id)
    result = llm.invoke([HumanMessage(content=prompt)])
...
def _plan_tools_with_llm(
    ...
    """
    return ds_planner.parse_planner_response(
        _invoke_planner_prompt(
            ds_planner.build_plan_prompt(message, tools_schema, dataset, conversation_history),
            model_id,
        ),
...
def _validate_tool_plan(plan: Dict[str, Any], tool_names: List[str]) -> Dict[str, Any]:
    ...
    """
    return ds_planner.validate_tool_plan(plan, tool_names)
...
def _repair_tool_plan(
    ...
    return ds_planner.parse_planner_response(
        _invoke_planner_prompt(
            ds_planner.build_repair_prompt(raw_plan, message, [tool["name"] for tool in tools_schema]),
            model_id,
        ),
        _parse_json_response,
...
def _resolve_tool_plan(
    ...
    return ds_planner.resolve_tool_plan(
        message=message,
        tools_schema=tools_schema,
        dataset=dataset,
        conversation_history=conversation_history,
        invoke_prompt=lambda prompt: _invoke_planner_prompt(prompt, model_id),
...
def run_data_scientist_analysis(
    ...
    if not tool_calls:
        fallback_tool_calls = _build_fallback_tool_calls(message)
    ...
    for call in tool_calls:
        ...
        resolved_tool_name = _resolve_tool_name(tool_name, chart_kind)
