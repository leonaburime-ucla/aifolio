...
def _stringify_content_parts(raw: list[Any]) -> str:
    """Flatten structured message content into a single string for parsing."""
    parts = []
    for item in raw:
...
def _strip_json_fence(text: str) -> str:
    """Remove a surrounding markdown code fence from JSON text."""
...
def _json_object_slice(text: str) -> str:
    """Slice the most likely top-level JSON object from a text blob."""
    start = text.find("{")
    end = text.rfind("}")
