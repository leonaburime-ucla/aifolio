...
def extract_text_from_llm_output(output: Any) -> str:
    ...
    if isinstance(output, dict):
        # Preserve JSON shape for downstream parsing instead of Python repr text.
        try:
...
def normalize_assistant_payload(raw_output: Any) -> dict[str, Any]:
    if isinstance(raw_output, dict):
        ...
        if isinstance(message, str):
            return {
                ...
                "chartSpec": normalize_chart_spec_payload(payload.get("chartSpec")),
                "actions": normalize_actions_payload(payload.get("actions")),
            }

    text = extract_text_from_llm_output(raw_output)
    parsed = parse_assistant_json(text)
    if not parsed:
        return {"type": "TextMessage", "message": text.strip(), "chartSpec": None, "actions": []}
    ...
    if not isinstance(message, str):
        message = text.strip()
    return {"type": "TextMessage", "message": message.strip(), "chartSpec": chart_spec, "actions": actions}
...
def format_assistant_json_text(raw_output: Any) -> str:
    ...
    { "type": "TextMessage", "message": string, "chartSpec": ChartSpec | ChartSpec[] | null }
    """
