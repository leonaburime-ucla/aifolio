...
def _temperature_scaled_probabilities(probabilities: np.ndarray | tf.Tensor, temperature: float) -> tf.Tensor:
    """Apply temperature scaling to probability outputs."""
    probs = tf.convert_to_tensor(probabilities, dtype=tf.float32)
    return tf.nn.softmax(tf.math.log(tf.clip_by_value(probs, 1e-7, 1.0)) / temperature, axis=1)


def _regression_rmse(preds: np.ndarray, y_true: np.ndarray, target_scaler: Any | None) -> float:
    """Compute regression RMSE in original target scale when needed."""
    ...
    return float(np.sqrt(np.mean((preds_original - y_true_original) ** 2)))
...
def distill_model_from_file(
        ...
        for _ in range(cfg.epochs):
            ...
            for start in range(0, len(idx), cfg.batch_size):
                ...
                with tf.GradientTape() as tape:
                    ...
                    teacher_temp = _temperature_scaled_probabilities(teacher_probs, temperature)
                    student_temp = _temperature_scaled_probabilities(student_probs, temperature)
    ...
    if task == "classification":
        ...
        rmse = _regression_rmse(preds, y_test, teacher.target_scaler)
        metrics = Metrics(
            task=task,
            train_loss=train_loss,
            test_loss=test_loss,
