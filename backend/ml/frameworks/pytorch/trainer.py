...
def _build_tree_teacher_targets(
    ...
    if task == "classification":
        ...
        teacher = classifier_cls(n_estimators=120, max_depth=8, random_state=random_seed, n_jobs=-1)
        teacher.fit(x_train_np, y_train_np)
        probs = teacher.predict_proba(x_train_np)
        return torch.tensor(np.clip(probs, 1e-6, 1.0), dtype=torch.float32, device=torch_device), None

    y_train_np = y_train.detach().cpu().numpy().reshape(-1)
    teacher = regressor_cls(n_estimators=120, max_depth=10, random_state=random_seed, n_jobs=-1)
    teacher.fit(x_train_np, y_train_np)
    preds = teacher.predict(x_train_np).reshape(-1, 1)
    return None, torch.tensor(preds, dtype=torch.float32, device=torch_device)


def _regression_predictions_to_list(preds: np.ndarray, target_scaler: Any | None) -> list[Any]:
    """Convert regression model outputs into response payload values."""
