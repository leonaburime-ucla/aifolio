...
def _regression_rmse(
    ...
    if target_scaler is not None:
        ...
        y_true_original = target_scaler.inverse_transform(y_true_np)
    else:
...
def _classification_soft_loss(
    student_out: torch.Tensor,
    teacher_out: torch.Tensor,
    temperature: float,
) -> torch.Tensor:
    """Compute temperature-scaled KL loss for classification distillation."""
...
def distill_model_from_file(
    ...
    for _ in range(cfg.epochs):
        for batch in batch_indices(len(x_train), cfg.batch_size):
            ...
            if task == "classification":
                ...
                soft_loss = _classification_soft_loss(student_out, teacher_out, temperature)
            else:
                hard_loss = hard_criterion(student_out, yb)
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            rmse = _regression_rmse(preds_np, y_test_np_t, teacher.target_scaler)
            metrics = Metrics(
                task=task,
                train_loss=float(train_loss),
                test_loss=float(test_loss),
