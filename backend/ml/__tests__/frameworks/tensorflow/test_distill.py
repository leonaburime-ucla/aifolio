...
from ml.frameworks.tensorflow import distill
from ml.frameworks.tensorflow.distill import distill_model_from_file
...
def _build_tf_teacher_bundle(task: str, use_target_scaler: bool = True) -> ModelBundle:
    ...
    if task == "classification":
        ...
        model(np.zeros((1, 1), dtype=np.float32))
        return ModelBundle(
            model=model,
            task="classification",
            ...
            output_dim=2,
            class_names=["no", "yes"],
            model_config={"hidden_dim": 8, "num_hidden_layers": 2, "dropout": 0.1},
        )
...
def test_temperature_scaled_probabilities_preserve_distribution_shape():
    scaled = distill._temperature_scaled_probabilities(np.array([[0.2, 0.8]], dtype=np.float32), temperature=2.0)
    scaled_np = scaled.numpy()
    assert scaled_np.shape == (1, 2)
    assert scaled_np.sum() == pytest.approx(1.0)
    assert 0.0 < scaled_np[0, 0] < scaled_np[0, 1] < 1.0
...
def test_distill_classification_end_to_end_with_teacher_bundle(tmp_path):
    ...
    bundle, metrics = distill_model_from_file(path, cfg, teacher_bundle=_build_tf_teacher_bundle("classification"))
    assert bundle.task == "classification"
    assert metrics.test_metric_name == "accuracy"
    assert bundle.model_config["hidden_dim"] == 16
...
def test_distill_regression_end_to_end_without_target_scaler(tmp_path):
    ...
    bundle, metrics = distill_model_from_file(path, cfg, teacher_bundle=_build_tf_teacher_bundle("regression", use_target_scaler=False))
...
def test_distill_regression_end_to_end_with_target_scaler_and_excluded_columns(tmp_path):
    ...
    bundle, metrics = distill_model_from_file(
        path,
        cfg,
        teacher_bundle=_build_tf_teacher_bundle("regression", use_target_scaler=True),
...
def test_distill_loads_teacher_bundle_from_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(distill, "load_bundle", lambda teacher_path: _build_tf_teacher_bundle("classification"))

    bundle, metrics = distill_model_from_file(path, cfg, teacher_path="teacher.keras")
    assert bundle.task == "classification"
    assert metrics.test_metric_name == "accuracy"
...
def test_distill_classification_requires_teacher_label_encoder(tmp_path):
    path = tmp_path / "classification.csv"
    _write_csv(path, [(0.0, "no"), (1.0, "yes"), (0.1, "no"), (1.1, "yes")])
    teacher = _build_tf_teacher_bundle("classification")
...
def test_distill_rejects_excluding_target_column(tmp_path):
    ...
    with pytest.raises(ValueError, match="target_column cannot be excluded"):
        distill_model_from_file(
            path,
            cfg,
            teacher_bundle=_build_tf_teacher_bundle("classification"),
