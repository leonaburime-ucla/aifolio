...
- Type contracts under `features/ai-chat/__types__`, with shared chart contracts under `features/charts/contracts`.
- Feature-local feedback rendering via `react/views/components/UIFeedback.tsx`.
...
- AB-003: `logic/*` MUST remain framework-agnostic and side-effect-light except explicit I/O adapters.
...
- REQ-005: AI chat contracts (chat/api + logic input/output types) MUST live under `features/ai-chat/__types__`; shared chart contracts MUST live under `features/charts/contracts`.
...
- AC-005 (REQ-005): Given ai-chat module imports contract types, they resolve from `@/features/ai-chat/__types__/*` or the shared chart contract at `@/features/charts/contracts/chart.types`.
