...
export function useAgenticResearchActions(): AgenticResearchActions {
  const current = agenticResearchStore.getState();
  return {
    setDatasetManifest: current.setDatasetManifest,
    setSelectedDatasetId: current.setSelectedDatasetId,
    setSklearnTools: current.setSklearnTools,
    setTableRows: current.setTableRows,
    setTableColumns: current.setTableColumns,
    setNumericMatrix: current.setNumericMatrix,
    setFeatureNames: current.setFeatureNames,
    setPcaChartSpec: current.setPcaChartSpec,
