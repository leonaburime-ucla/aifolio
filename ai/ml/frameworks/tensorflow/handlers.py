...
def handle_train_request(
    ...
    if training_mode not in TENSORFLOW_ALLOWED_TRAINING_MODES:
        return 400, {"status": "error", "error": allowed_modes_error(sorted(TENSORFLOW_ALLOWED_TRAINING_MODES))}
...
def _handle_distill_request_impl(
    ...
    teacher_model_path = resolve_teacher_model_path(payload, artifacts_dir, "model_bundle.keras")
    ...
    if training_mode not in TENSORFLOW_ALLOWED_TRAINING_MODES:
        return 400, {"status": "error", "error": allowed_modes_error(sorted(TENSORFLOW_ALLOWED_TRAINING_MODES))}
    if training_mode in TENSORFLOW_UNSUPPORTED_DISTILL_MODES:
        return 400, {
            "status": "error",
            "error": unsupported_distill_mode_error(training_mode, "mlp_dense, linear_glm_baseline, or wide_and_deep"),
        }
