...
def test_chat_route_uses_mock_llm(monkeypatch):
    ...
    monkeypatch.setattr("backend.server.http.run_unified_chat", _fake_unified_chat)
...
def test_run_chat_research_normalizes_message_and_actions(monkeypatch):
    monkeypatch.setattr(
        "backend.server.http.run_unified_chat",
        lambda payload: ("provider", {"message": None, "chartSpec": None}),
    )
    from backend.server.http import _run_chat_research
...
def test_chat_get_uses_default_model(monkeypatch):
    monkeypatch.setattr("backend.server.http.langchain_chat_response", lambda payload: payload)
...
def test_agui_route_returns_stream_response(monkeypatch):
    monkeypatch.setattr("backend.server.http.create_agui_stream_response", lambda payload: {"stream": payload})
...
def test_llm_ds_route_uses_coordinator_when_dataset_present(monkeypatch):
    monkeypatch.setattr("backend.server.http.run_unified_chat", lambda payload: ("coordinator", {"message": "ok"}))
...
def test_llm_ds_route_uses_direct_tool_mode(monkeypatch):
    ...
    monkeypatch.setattr("backend.server.http.run_data_scientist_tool", _run_tool)
...
def test_llm_ds_route_uses_chat_mode_without_dataset_or_tool(monkeypatch):
    monkeypatch.setattr(
        "backend.server.http.run_data_scientist",
...
def test_llm_ds_get_returns_demo_payload(monkeypatch):
    monkeypatch.setattr("backend.server.http.run_demo_pca_transform", lambda n_components: {"components": n_components})
...
def test_agent_status_route_returns_status(monkeypatch):
    monkeypatch.setattr("backend.server.http.get_status", lambda: {"agents": "ok"})
...
def test_langsmith_trace_route_returns_404_for_error_report(monkeypatch):
    monkeypatch.setattr("backend.server.http.get_trace_report", lambda **kwargs: {"status": "error", "error": "not found"})
...
def test_langsmith_trace_route_returns_500_on_exception(monkeypatch):
    monkeypatch.setattr("backend.server.http.get_trace_report", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("boom")))
    response = client.get("/llm/langsmith/trace/trace-1")
...
def test_sample_data_routes_handle_success_and_not_found(monkeypatch):
    monkeypatch.setattr("backend.server.http.list_sample_datasets", lambda: [{"id": "s1"}])
    monkeypatch.setattr("backend.server.http.load_sample_dataset", lambda dataset_id: {"status": "error", "error": "missing"})
...
def test_ml_data_detail_returns_404_for_missing_dataset(monkeypatch):
    monkeypatch.setattr(
        "backend.server.http.load_ml_dataset",
...
def test_sklearn_tools_route_returns_tools_and_schemas(monkeypatch):
    monkeypatch.setattr("backend.server.http.sklearn_tools.list_available_tools", lambda: ["a"])
    monkeypatch.setattr("backend.server.http.sklearn_tools.get_tools_schema", lambda: {"a": {}})
...
def test_training_and_distill_routes_delegate_to_shared_endpoint(monkeypatch, path, expected_framework, expected_artifact):
    monkeypatch.setattr(
        "backend.server.http.run_training_or_distill_endpoint",
...
def test_predict_routes_delegate_to_shared_endpoint(monkeypatch, path, framework):
    monkeypatch.setattr(
        "backend.server.http.run_predict_endpoint",
...
def test_framework_status_routes_delegate(monkeypatch, path, package):
    monkeypatch.setattr(
        "backend.server.http.framework_status",
