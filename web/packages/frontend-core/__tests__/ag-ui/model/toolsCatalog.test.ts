...
import { getAgUiToolsForTab } from "@aifolio/frontend-core/ag-ui";

describe("getAgUiToolsForTab", () => {
  it("includes chart tools on the base charts tab", () => {
    const tools = getAgUiToolsForTab("charts");
    const names = tools.map((t) => t.name);
    expect(names).toContain("switch_ag_ui_tab");
    expect(names).toContain("add_chart_spec");
    expect(names).toContain("clear_charts");
    expect(names).not.toContain("set_pytorch_form_fields");
  });
