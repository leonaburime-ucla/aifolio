...
describe("handleNavigateToPage", () => {
  it("resolves route aliases and slash-prefixed passthrough values", () => {
    expect(resolveRouteAlias(" AI Chat ")).toBe("/");
    expect(resolveRouteAlias("/custom")).toBe("/custom");
    expect(resolveRouteAlias("unknown")).toBe("");
    expect(resolveRouteAlias("")).toBe("");
    expect(isAllowedRoute("/ag-ui")).toBe(true);
    expect(isAllowedRoute("/custom")).toBe(false);
  });

  it("returns ok for supported alias", () => {
    const result = handleNavigateToPage("pytorch");
    expect(result).toEqual({ status: "ok" as const, resolvedRoute: "/ml/pytorch" });
  });
