...
describe("agentic chartTools", () => {
  it("adds normalized chart specs and returns ids", () => {
    ...
    ).toEqual({ status: "ok" as const, addedCount: 1, ids: ["a"] });
    expect(
      handleAgenticAddChartSpec(
        { chartSpecs: [chart("a"), chart("b")] },
        addChartSpec
      )
    ).toEqual({ status: "ok" as const, addedCount: 2, ids: ["a", "b"] });

    expect(addChartSpec).toHaveBeenCalledTimes(3);
  });

  it("returns INVALID_CHART_SPEC when payload cannot be normalized", () => {
    const result = handleAgenticAddChartSpec({ chartSpec: { invalid: true } }, vi.fn());
    expect(result).toEqual({ status: "error" as const, code: "INVALID_CHART_SPEC", addedCount: 0 });
  });

  it("clears charts and removes existing ids", () => {
    const clearFn = vi.fn();
    expect(handleAgenticClearCharts(clearFn)).toEqual({ status: "ok" as const, cleared: true });
    ...
    ).toEqual({ status: "ok" as const, removed_chart_id: "a", remaining_count: 1 });
  ...
  it("reorders by ordered ids or index move and validates payload", () => {
    ...
    ).toEqual({ status: "ok" as const, mode: "ordered_ids", chart_ids: ["c", "a", "b"] });
    ...
    ).toEqual({ status: "ok" as const, mode: "index_move", chart_ids: ["b", "c", "a"] });
