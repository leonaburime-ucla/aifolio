...
import { handleAgenticSetActiveDataset } from "@aifolio/frontend-core/agentic-research";
...
describe("agentic datasetTools", () => {
  it("accepts exact id and preserves existing charts while setting dataset", () => {
    ...
    ).toEqual({ status: "ok" as const, active_dataset_id: "iris.csv" });
  ...
  it("resolves dataset by normalized/fuzzy label tokens", () => {
    ...
    ).toEqual({ status: "ok" as const, active_dataset_id: "wine-quality.csv" });
