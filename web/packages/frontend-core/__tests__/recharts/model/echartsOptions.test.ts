...
function firstSeries(option: ReturnType<typeof getEChartsOption>) {
  const series = option?.series;
  if (Array.isArray(series)) return series[0] as Record<string, unknown>;
  return series as Record<string, unknown> | undefined;
}
...
describe("echartsOptions.logic", () => {
  it("builds heatmap option and coerces invalid cells to zero", () => {
    ...
    expect(firstSeries(option)).toMatchObject({ type: "heatmap" });
    expect((firstSeries(option) as { data: number[][] }).data).toEqual([
      [0, 0, 1],
      [0, 1, 0],
    ]);
  });
  ...
  it("builds boxplot and bar/histogram options", () => {
    ...
    expect(firstSeries(box)).toMatchObject({ type: "boxplot", data: [[1, 2]] });
    ...
    expect(firstSeries(bar)).toMatchObject({ type: "bar", data: [1, 2] });
    ...
    expect(firstSeries(histogram)).toMatchObject({ type: "bar", data: [3] });
  });
  ...
  it("builds tree option for dendrogram with rooted and parent-child nodes", () => {
    ...
    expect(firstSeries(option)).toMatchObject({ type: "tree" });
    const treeData = (firstSeries(option) as { data: Array<{ name: string; children?: unknown[] }> }).data;
  ...
  it("ignores dendrogram rows without id/label/name", () => {
    ...
    const treeData = (firstSeries(option) as { data: Array<{ name: string }> }).data;
  ...
  it("builds scatter/biplot options including tooltip and feature labels", () => {
    ...
    expect(firstSeries(scatter)).toMatchObject({ type: "scatter" });
    ...
    expect(firstSeries(biplot)).toMatchObject({ type: "scatter" });
