...
describe("distillationView.logic", () => {
  ...
  it("falls back to result metric name when teacher metric name is missing", () => {
    const built = buildDistillationComparison({
      teacher: {
        metric_score: null,
      },
      result: {
        metrics: { test_metric_name: "f1_score", test_metric_value: 0.7 },
        distilledRun: { training_mode: "" },
        teacherInputDim: null,
        teacherOutputDim: null,
        studentInputDim: null,
        studentOutputDim: null,
    ...
    expect(built.teacherMetricName).toBe("f1_score");
    expect(built.teacherMetricValue).toBeNull();
    expect(built.qualityDelta).toBeNull();
    expect(built.comparison.teacherTrainingMode).toBeNull();
    expect(built.comparison.studentTrainingMode).toBeNull();
  });

  it("falls back to accuracy when teacher and result metric names are missing", () => {
    const built = buildDistillationComparison({
      teacher: {
        metric_name: null,
        metric_score: "0.7",
    ...
    const fallback = resolveDistilledModalPayload({
      ...
      trainingRuns: [
        ...
        },
        {
          ...
          distill_teacher_training_mode: null,
          distill_student_training_mode: null,
        },
      ],
    });
