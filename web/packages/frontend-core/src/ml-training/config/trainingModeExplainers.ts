import type { PytorchTrainingMode, TensorflowTrainingMode } from "@aifolio/contracts/entities/ml-training";
...
/**
 * Looks up explanatory copy for a PyTorch training mode.
 *
 * @param mode - Training mode identifier.
 * @returns Mode explainer copy, or a fallback explainer for unknown modes.
...
/**
 * Looks up explanatory copy for a TensorFlow training mode.
 *
 * @param mode - Training mode identifier.
 * @returns Mode explainer copy, or a fallback explainer for unknown modes.
 * @complexity O(1) time and space.
