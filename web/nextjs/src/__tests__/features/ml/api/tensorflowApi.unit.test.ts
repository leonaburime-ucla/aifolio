...
describe("tensorflowApi", () => {
  ...
  it("returns error payload for unsuccessful train response", async () => {
    const fetchImpl = vi.fn(async () => ({
      ok: false,
      json: async () => ({ status: "error" as const, error: "bad train" }),
    }));
    ...
    const nonOk = await trainTensorflowModel(
      { dataset_id: "d1.csv", target_column: "target" },
      {
        fetchImpl: vi.fn(async () => ({
          ok: true,
          json: async () => ({ status: "error" as const }),
  ...
  it("returns ok payload for successful distill response and clears timeout", async () => {
    const fetchImpl = vi.fn(async () => ({
      ok: true,
      json: async () => ({ status: "ok" as const, run_id: "distill-1" }),
    }));
    ...
    const result = await distillTensorflowModel(
      ...
      {
        fetchImpl: fetchImpl as unknown as typeof fetch,
        scheduleTimeout: scheduleTimeout as unknown as typeof setTimeout,
  ...
  it("returns request failure with message when distill throws a non-abort Error", async () => {
    ...
    const result = await distillTensorflowModel(
      ...
      {
        fetchImpl: fetchImpl as unknown as typeof fetch,
        scheduleTimeout: vi.fn(() => 222 as unknown as ReturnType<typeof setTimeout>) as unknown as typeof setTimeout,
  ...
  it("uses default distill failure messages for response and non-Error throws", async () => {
    const failed = await distillTensorflowModel(
      { dataset_id: "d1.csv", target_column: "target" },
      {
        ...
        scheduleTimeout: vi.fn(() => 1 as unknown as ReturnType<typeof setTimeout>) as unknown as typeof setTimeout,
        clearScheduledTimeout: vi.fn(),
      }
    );
    ...
    const thrown = await distillTensorflowModel(
      { dataset_id: "d1.csv", target_column: "target" },
      {
        ...
        scheduleTimeout: vi.fn(() => 2 as unknown as ReturnType<typeof setTimeout>) as unknown as typeof setTimeout,
        clearScheduledTimeout: vi.fn(),
      }
    );
    expect(thrown).toEqual({
