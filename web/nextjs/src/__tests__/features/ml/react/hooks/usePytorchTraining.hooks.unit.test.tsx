...
import {
  usePytorchLogic,
  usePytorchUiState,
} from "@/features/ml/react/hooks/usePytorchTraining.hooks";
import type { PytorchLogicArgs } from "@/features/ml/react/hooks/usePytorchTraining.hooks.types";
...
describe("usePytorchTraining.hooks", () => {
  ...
  it("handles unsupported/no-dataset/error distillation branches", async () => {
    ...
    const runDistillation = vi.fn(async () => ({ status: "error" as const, error: "distill failed" }));
    const { result } = renderHook(() =>
      usePytorchLogic({
  ...
  it("falls back to current ui mode when teacher training mode is missing", async () => {
    ...
    expect(runDistillation).toHaveBeenCalledWith(
      expect.objectContaining({ trainingMode: "mlp_dense" }),
      expect.anything()
    );
  });
  ...
  it("updates shouldContinue when stop is requested during an active run", async () => {
    ...
    type CapturedTrainingDeps = {
      shouldContinue: () => boolean;
      onProgress: (current: number, total: number) => void;
    };
    const requireCapturedTrainingDeps = (
      deps: CapturedTrainingDeps | null
    ): CapturedTrainingDeps => {
      if (!deps) throw new Error("Expected training dependencies to be captured.");
      return deps;
    };
    let capturedDeps: CapturedTrainingDeps | null = null;
    let releaseRun: () => void = () => {};
    ...
    const activeDeps = requireCapturedTrainingDeps(capturedDeps);
    expect(activeDeps.shouldContinue()).toBe(true);

    act(() => {
      result.current.onStopTrainingRuns();
    });
    expect(activeDeps.shouldContinue()).toBe(false);
  ...
  it("normalizes n/a teacher references before distillation request", async () => {
    ...
    expect(runDistillation).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
