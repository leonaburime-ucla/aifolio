...
} from "@/features/ml/react/hooks/useTensorflowTraining.hooks";
import type { TensorflowLogicArgs } from "@/features/ml/react/hooks/useTensorflowTraining.hooks.types";
...
describe("useTensorflowTraining.hooks", () => {
  ...
  it("handles unsupported/no-dataset/error distillation branches", async () => {
    ...
    const runDistillation = vi.fn(async () => ({ status: "error" as const, error: "tf distill failed" }));
  ...
  it("falls back to current ui mode when teacher training mode is missing", async () => {
    ...
    expect(runDistillation).toHaveBeenCalledWith(
      expect.objectContaining({ trainingMode: "wide_and_deep" }),
      expect.anything()
    );
  });
