...
function buildNumericUi(): NumericInputState {
  return {
    epochValuesInput: "60",
    batchSizesInput: "64",
...
function buildOptimizerUi(
  overrides: Partial<OptimizerUiState> = {}
): OptimizerUiState {
  return {
...
describe("trainingShared.logic", () => {
  ...
  it("applies pending optimal params to numeric inputs", () => {
    const ui: HandleApplyOptimalParamsUi = {
  ...
  it("copies explicit distill_action values without replacing them", async () => {
    ...
    const copiedTsv = String(writeClipboardText.mock.calls[0]?.[0] ?? "");
