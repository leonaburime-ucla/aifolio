...
} from "@/__tests__/features/ml/test-utils/trainingOrchestrator.test-utils";
...
describe("pytorchTraining.orchestrator", () => {
  ...
  it("returns error status when distillation fails", async () => {
    const result = await runPytorchDistillation(
      ...
      {
        distillModel: vi.fn(async () => ({ status: "error" as const, error: "distill failed" })),
        ...defaultFormatters,
      }
    );

    expect(result).toEqual({ status: "error" as const, error: "distill failed" });
  });
  ...
  it("returns default distillation error when backend omits error", async () => {
    const result = await runPytorchDistillation(
      ...
      {
        distillModel: vi.fn(async () => ({ status: "error" as const })),
        ...defaultFormatters,
      }
    );

    expect(result).toEqual({ status: "error" as const, error: "Distillation failed." });
  });
