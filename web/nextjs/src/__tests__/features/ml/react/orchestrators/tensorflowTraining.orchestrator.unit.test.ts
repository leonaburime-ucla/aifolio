...
describe("tensorflowTraining.orchestrator", () => {
  ...
  it("uses default failure text and n/a baseline values", async () => {
    ...
    const result = await runTensorflowTraining(
      ...
      {
        trainModel: vi.fn(async () => ({ status: "error" as const })),
        prependTrainingRun,
        onProgress: vi.fn(),
        ...defaultFormatters,
      }
  ...
  it("returns default distillation error and clamps student config", async () => {
    ...
    expect(err).toEqual({ status: "error" as const, error: "Distillation failed." });

    const distillModel = vi.fn(async () => ({ status: "ok" as const, metrics: {} }));
