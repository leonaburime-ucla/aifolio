...
describe("waitForPytorchForm", () => {
  it("returns true when pytorch form field is present", async () => {
    (window as Window & { __AIFOLIO_PYTORCH_FORM_BRIDGE__?: PytorchFormBridge }).__AIFOLIO_PYTORCH_FORM_BRIDGE__ =
      { applyPatch: () => ({ applied: [], skipped: [] }) };
    ...
    delete (window as Window & { __AIFOLIO_PYTORCH_FORM_BRIDGE__?: PytorchFormBridge })
...
describe("waitForTensorflowForm", () => {
  it("returns true when tensorflow form field is present", async () => {
    (window as Window & { __AIFOLIO_TENSORFLOW_FORM_BRIDGE__?: TensorflowFormBridge }).__AIFOLIO_TENSORFLOW_FORM_BRIDGE__ =
      { applyPatch: () => ({ applied: [], skipped: [] }) };
    ...
    expect(result).toBe(true);
    delete (window as Window & { __AIFOLIO_TENSORFLOW_FORM_BRIDGE__?: TensorflowFormBridge })
