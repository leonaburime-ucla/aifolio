...
vi.mock("@copilotkit/react-ui", () => ({
  AssistantMessage: ({ message }: AssistantMessageProps) => (
    <div data-testid="assistant-message">{typeof message?.content === "string" ? message.content : ""}</div>
  ),
}));

vi.mock("@aifolio/frontend-core/ag-ui", async (importOriginal) => {
...
} from "@/features/ag-ui-chat/react/views/components/CopilotAssistantMessage";
...
describe("CopilotAssistantMessage component", () => {
  it("renders transformed content with tool trace", () => {
    const props = {
      ...
      isGenerating: false,
      rawData: undefined,
    } satisfies AssistantMessageProps;
