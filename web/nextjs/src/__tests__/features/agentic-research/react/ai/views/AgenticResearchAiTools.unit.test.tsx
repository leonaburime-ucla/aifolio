...
const handlers = {
  addChartSpec: vi.fn(() => ({ status: "ok" as const })),
  clearCharts: vi.fn(() => ({ status: "ok" as const })),
  removeChartSpec: vi.fn(() => ({ status: "ok" as const })),
  reorderChartSpecs: vi.fn(() => ({ status: "ok" as const })),
  setActiveDataset: vi.fn(() => ({ status: "ok" as const })),
};
...
vi.mock("@/features/agentic-research/react/ai/adapters/useAgenticResearchAiSurface", () => ({
