...
import {
  useChatSidebarUi,
  type ChatSidebarUiDeps,
  type ChatSidebarUi as ChatSidebarUiModel,
} from "@/features/ai-chat/react/hooks/useChatSidebar.web";
import UIFeedback from "@/features/ai-chat/react/views/components/UIFeedback";
...
type ChatSidebarProps = {
  ...
  useSidebarUi?: (deps: ChatSidebarUiDeps) => ChatSidebarUiModel;
  className?: string;
};

export default function ChatSidebar({
  chatOrchestrator = useChatOrchestrator,
  useSidebarUi = useChatSidebarUi,
  className = "",
}: ChatSidebarProps) {
  const {
    messages,
  ...
  const {
    ...
    handleDragOver,
    handleDragLeave,
  } = useSidebarUi({ messages, isSending, addAttachments });

  return (
    <aside
      className={`relative flex h-[calc(100vh-64px)] w-full flex-col border-l border-zinc-200 bg-white ${className}`}
    ...
    >
      ...
      <div className="border-t border-zinc-200 bg-white">
        <ChatBar mode="embedded" chatOrchestrator={chatOrchestrator} />
      </div>
    </aside>
  );
}
