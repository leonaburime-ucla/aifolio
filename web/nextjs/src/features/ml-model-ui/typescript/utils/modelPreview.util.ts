...
export function parseLayerBullet(bullet: string): { term: string; definition: string } {
  ...
  return {
    term: bullet.slice(0, separatorIndex).trim(),
    definition: bullet.slice(separatorIndex + 1).trim(),
  };
}
...
export function buildModelPreviewGraph({
  framework,
  mode,
}: {
  ...
  return buildGraph(framework, mode);
}

export function buildModelPreviewBullets({
  mode,
}: {
  mode: ModelPreviewMode;
}): ModelPreviewBullets {
  return buildLayerBullets(mode);
}
