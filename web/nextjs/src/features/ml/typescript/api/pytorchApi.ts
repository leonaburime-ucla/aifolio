...
export async function trainPytorchModel(
  ...
  }: Partial<PytorchApiRuntime> = {}
): Promise<PytorchTrainSuccess | PytorchTrainError> {
  try {
    const response = await fetchImpl(`${resolveBaseUrl()}/ml/pytorch/train`, {
      method: "POST",
...
export async function distillPytorchModel(
  ...
  try {
    const response = await fetchImpl(`${resolveBaseUrl()}/ml/pytorch/distill`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
