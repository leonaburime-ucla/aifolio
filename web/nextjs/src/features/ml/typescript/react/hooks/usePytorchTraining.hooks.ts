...
import { distillPytorchModel, trainPytorchModel } from "@/features/ml/typescript/api/pytorchApi";
...
const DEFAULT_PYTORCH_RUNTIME: PytorchRuntimeDeps = {
...
export function usePytorchLogic({
  ...
  const runtimeDeps: PytorchRuntimeDeps = { ...DEFAULT_PYTORCH_RUNTIME, ...runtime };
  ...
  const isDistillationSupported = isPytorchDistillSupportedMode(ui.trainingMode);
  ...
  async function runDistillationFromTeacher(
    ...
    if (!isPytorchDistillSupportedMode(teacherTrainingMode)) {
      ui.setTrainingError(
    ...
    const result = await runDistillation(
      {
        ...
        targetColumn: resolvedTargetColumn.trim(),
        task: ui.task,
        trainingMode: teacherTrainingMode as PytorchTrainingMode,
  ...
  return {
    ...
    isDistillationSupportedForRun: (run: TrainingRunRow) =>
      isPytorchDistillSupportedMode(String(run.training_mode ?? "")),
  };
}
