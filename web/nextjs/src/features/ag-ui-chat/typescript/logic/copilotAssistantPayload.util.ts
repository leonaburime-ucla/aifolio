...
import type { CopilotAssistantPayload } from "@/features/ag-ui-chat/__types__/typescript/logic/copilotAssistantPayload.types";
...
export function parseCopilotAssistantPayload(rawContent: string): CopilotAssistantPayload | null {
  ...
  const payload: CopilotAssistantPayload = {
    type: parsed.type === "TextMessage" ? "TextMessage" : undefined,
    message,
...
/**
 * Normalizes Copilot assistant message content for legacy rendering.
 *
 * @param message Incoming Copilot message payload.
 * @returns Message with transport JSON stripped from string content.
 */
