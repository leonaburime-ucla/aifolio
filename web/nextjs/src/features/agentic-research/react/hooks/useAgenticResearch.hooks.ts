...
export type AgenticResearchLogicRuntimeDeps = {
  debugEffects?: boolean;
};
...
export function useAgenticResearchLogic(
  ...
  runtimeDeps?: AgenticResearchLogicRuntimeDeps
): {
  reloadManifest: () => void;
  setSelectedDatasetId: AgenticResearchActions["setSelectedDatasetId"];
} {
  const debugEffects =
    runtimeDeps?.debugEffects ?? process.env.NEXT_PUBLIC_DEBUG_EFFECTS === "1";
