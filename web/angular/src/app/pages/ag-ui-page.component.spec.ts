...
describe('ChartStoreService', () => {
  let chartStore: ChartStoreService;

  beforeEach(() => {
    TestBed.configureTestingModule({});
    chartStore = TestBed.inject(ChartStoreService);
  });

  it('starts empty', () => {
    expect(chartStore.chartSpecs()).toEqual([]);
  });

  it('adds a chart spec', () => {
    chartStore.addChartSpec(chartSpec);
    expect(chartStore.chartSpecs().length).toBe(1);
    expect(chartStore.chartSpecs()[0]).toEqual(chartSpec);
  });

  it('deduplicates by id (newest first)', () => {
    chartStore.addChartSpec(chartSpec);
    const updated = { ...chartSpec, title: 'Updated Revenue' };
    chartStore.addChartSpec(updated);
    expect(chartStore.chartSpecs().length).toBe(1);
    expect(chartStore.chartSpecs()[0].title).toBe('Updated Revenue');
  });

  it('clears all charts', () => {
    chartStore.addChartSpec(chartSpec);
    chartStore.addChartSpec({ ...chartSpec, id: 'chart-b' });
    chartStore.clearCharts();
    expect(chartStore.chartSpecs()).toEqual([]);
  });

  it('removes a specific chart by id', () => {
    chartStore.addChartSpec(chartSpec);
    chartStore.addChartSpec({ ...chartSpec, id: 'chart-b', title: 'Other' });
    chartStore.removeChartSpec('chart-a');
    expect(chartStore.chartSpecs().length).toBe(1);
    expect(chartStore.chartSpecs()[0].id).toBe('chart-b');
  });
});

describe('chart bridge integration', () => {
  let chartStore: ChartStoreService;
